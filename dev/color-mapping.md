...
| — | `hsl(207, 77%, 44%)` | `--grid-current-accent` — a current-size cell that is also inside the picked block. The accent one step down, the same step `#md-resize-grid:hover` takes; the picked block is solid accent, so the cells the table already has need their own value or the current size vanishes inside the selection |
...
| — | `hsl(208, 64%, 36%)` | `--grid-current-accent` — one step below dark's accent; deeper than light's equivalent step because a dark surface flattens the difference |
