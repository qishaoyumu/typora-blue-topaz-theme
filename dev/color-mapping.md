...
| `--table-hover-raw-color` | `#85bfee2f` | `--table-hover-raw-color` — a hovered body row, replacing its zebra fill; byte-identical upstream. Lifted from section 12's `tr:hover` literal into a token because section 13 paints the same colour on the source row of a drag, which is not :hover |
...
| — | `#f1f1f176` / `#ffffff70` (odd) | `--bt-row-base` — a body row's resting zebra fill, lifted into a token so the dark file overrides the two parities alone. Port-internal; the values are section 12's own, unchanged (it once fed section 13's handle-hover overlay, dropped 2026-09-12) |
...
| `--theme-color-translucent-*` family (no reference control) | `hsla(207, 77%, 54%, 0.24)` | `--grid-current-bg` — picker body cells marking the table's current size, the base blue formula as a tint, a step above `--suggest-active-bg`'s 0.15 because a 13px chip has less area to read from. main.js marks the `<td>` (`td.md-grid-ext`) but the fill is painted on the `<a>` inside it, so one box owns both radius and fill. A picked cell is solid `--interactive-accent`; a picked cell the table already has keeps this tint under a 2px inset ring of the solid, so the current size stays readable inside a larger pick, and the tint left standing outside a smaller pick shows a shrink. Was two greys (0.22 / 0.34 header) plus a deeper picked-while-current blue until 2026-09-12 |
| — | `hsla(207, 77%, 54%, 0.55)` | `--grid-head-current-bg` — the header row inside the current size, two notches above the body's 0.24 |
| — | `hsl(207, 77%, 40%)` | `--grid-head-accent` — the header row inside the pick: the accent two notches deeper than `--interactive-accent` (58% lightness), and the ring colour of a picked header cell the table already has |
...
| — | `hsla(208, 64%, 49%, 0.36)` | `--grid-current-bg` — dark's base blue formula at 0.36, half again light's 0.24 because a dark surface swallows a wash (was white 0.28 / 0.40 header, plus a deeper picked-while-current blue) |
| — | `hsla(208, 64%, 49%, 0.70)` | `--grid-head-current-bg` — header row inside the current size, half again light's 0.55 |
| — | `hsl(208, 70%, 64%)` | `--grid-head-accent` — header row inside the pick: brighter than dark's 49% accent rather than deeper, since deeper reads as less on a dark surface |
