...
/* 外距挪到 figure，表格归零：前段 16 与 figure 20 塌陷成 20，等于 Obsidian。
   figure 贴着表格居中，给工具条一个等宽的包含块；宽表仍在 figure 内横向滚动。 */
...
- 现有 `#write table:not(.md-reset){margin:20px auto}` 的外距改为 `0`。编辑器与导出的表格都包在 `figure.table-figure` 里，间距统一由 figure 承担。`width:auto`、六值、阴影、守卫不动。
...
#write figure.table-figure > .ty-table-edit {
  width: auto !important;      /* 例外：压 JS 写的内联宽度（figure 宽加 10px）。CSS 里写明理由。 */
...
- 暗色覆盖：边框用 `--dark-border-color`；三个网格 token 与 `--table-drag-src-bg` 各给暗值。
...
/* hover：btn-default 叠 --item-hover-bg-color；btn-primary 沿用现有的深一档；暗色 btn-default 底 #2b2b2b */
```
