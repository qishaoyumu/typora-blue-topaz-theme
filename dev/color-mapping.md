...
| — | `hsla(207, 77%, 54%, 0.16)` | `--grid-head-bg` — the picker's header row (row 1, always the table's header) at rest: the table's own header wash, a notch above section 12's 0.1 so it reads on a 13px chip |
...
| — | `hsla(208, 64%, 49%, 0.20)` | `--grid-head-bg` — header row at rest, half again light's 0.16 |
